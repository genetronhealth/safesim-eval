...
### realdata
tfq1="${inputdir}/${t}/${t}_1.fastq.gz"
tfq2="${inputdir}/${t}/${t}_2.fastq.gz"
...
nfq1="${inputdir}/${n}/${n}_1.fastq.gz"
nfq2="${inputdir}/${n}/${n}_2.fastq.gz"

echo "source ${init} ctDNA
bash ${scriptdir}/consensus.sh -i ${nfq1} -j ${nfq2} -l ${lumi} -s ${n} -o ${outdir}/rmdup-sim/consensus" > "${outdir}/scriptlog/${n}_consensus.sh"

echo "source ${init} ctDNA
bash ${scriptdir}/safemut_eval.sh -n ${outdir}/rmdup-sim/consensus/${n}.consensus.align.bam -v ${simfiles}/${c}_simmut_safemut.vcf.gz -r ${simfiles}/${c}_simmut.bed -s ${n} -o ${outdir}/rmdup-sim/safemut/${n}
...
echo "source ${init} ctDNA
bash ${scriptdir}/varben_eval.sh -n ${outdir}/rmdup-sim/consensus/${n}.consensus.align.bam -v ${simfiles}/${c}_simmut_varben.tsv -r ${simfiles}/${c}_simmut.bed -s ${n} -o ${outdir}/rmdup-sim/varben/${n}
...
echo "source ${init} ctDNA
bash ${scriptdir}/bamsurgeon_eval.sh -n ${outdir}/rmdup-sim/consensus/${n}.consensus.align.bam -s ${simfiles}/${c}_simmut_bamsurgeon_snv.tsv -i ${simfiles}/${c}_simmut_bamsurgeon_indel.tsv -r ${simfiles}/${c}_simmut.bed -S ${n} -o ${outdir}/rmdup-sim/bamsurgeon/${n}
...
echo "source ${init} ctDNA
bash ${scriptdir}/safemut_eval.sh -n ${outdir}/sim-rmdup/nbam/${n}.bam -v ${simfiles}/${c}_simmut_safemut.vcf.gz -r ${simfiles}/${c}_simmut.bed -s ${n} -o ${outdir}/sim-rmdup/safemut/${n} -l ${lumi}
...
echo "source ${init} ctDNA
bash ${scriptdir}/varben_eval.sh -n ${outdir}/sim-rmdup/nbam/${n}.bam -v ${simfiles}/${c}_simmut_varben.tsv -r ${simfiles}/${c}_simmut.bed -s ${n} -o ${outdir}/sim-rmdup/varben/${n} -l ${lumi}
...
echo "paste ${outdir}/realdata/*_simmut.info.txt |cut -f 1,2,6,10,14,18,22,26,30,34,38,42,46,50,54,58,62,66,70,74,78,82,86,90,94 > ${outdir}/realdata/realdata.txt
...
done" > "${outdir}/scriptlog/run_plot.sh"
